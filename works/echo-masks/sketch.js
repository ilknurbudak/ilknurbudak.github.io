...
// Sahnenin yarısı (ayna ekseninin solu) ve tam yüksekliği.
let HW, HH;

function sahneOlcusu() {
  const el = document.getElementById('canvas-container');
  return {
    w: Math.max(200, el.clientWidth),
    h: Math.max(200, el.clientHeight)
  };
}

function setup() {
  const o = sahneOlcusu();
  let c = createCanvas(o.w, o.h);
  c.parent('canvas-container');

  HW = Math.floor(width / 2);
  HH = height;

  pgHalf = createGraphics(HW, HH);
...
// Pencere boyu değişince kâğıdı sahneye yeniden yay.
function windowResized() {
  const o = sahneOlcusu();
  resizeCanvas(o.w, o.h);
...
function kunyeYaz() {
  ...
  if (!k) return;
  k.querySelector('.model').textContent = currentPreset;
  k.querySelector('.seed').textContent = 'seed ' + seed;
}
...
function draw() {
  ...
  for (let p of particles) {
    ...
    if (p.life <= 0 || p.x < 0 || p.x > HW || p.y < 0 || p.y > HH) {
